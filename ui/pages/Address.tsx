...
import useUserProfileApiQuery from 'lib/hooks/useUserProfileApiQuery';
...
const userProfileAPIFeature = config.features.userProfileAPI;
...
const AddressPageContent = () => {
  ...
  const userPropfileApiQuery = useUserProfileApiQuery(hash, userProfileAPIFeature.isEnabled && areQueriesEnabled);
  ...
  const tags: Array<EntityTag> = React.useMemo(() => {
    return [
      ...
      userProfileAPIFeature.isEnabled && usernameApiTag ? {
        slug: 'username_api',
        name: usernameApiTag,
        tagType: 'custom' as const,
        ordinal: 11,
        meta: {
          tagIcon: userProfileAPIFeature.tagIcon,
          bgColor: userProfileAPIFeature.tagBgColor,
          textColor: userProfileAPIFeature.tagTextColor,
          tagUrl: userProfileAPIFeature.tagLinkTemplate ? userProfileAPIFeature.tagLinkTemplate.replace('{username}', usernameApiTag) : undefined,
        },
      } : undefined,
  ...
  const titleContentAfter = (
    <EntityTags
      ...
      isLoading={
        ...
        (userProfileAPIFeature.isEnabled && userPropfileApiQuery.isPending)
      }
    />
  );
